...
int PPPOSClient::connect(IPAddress ip, uint16_t port) {
  ...
  int res =
      lwip_connect(sockfd, (struct sockaddr *)&serveraddr, sizeof(serveraddr));
#else
  int res = lwip_connect_r(sockfd, (struct sockaddr *)&serveraddr,
                           sizeof(serveraddr));
#endif
  if (res < 0 && errno != EINPROGRESS) {
    log_e("connect on fd %d, errno: %d, \"%s\"", sockfd, errno,
          strerror(errno));
    close(sockfd);
    return 0;
  }
  ...
  res =
      select(sockfd + 1, nullptr, &fdset, nullptr, timeout < 0 ? nullptr : &tv);
  if (res < 0) {
    ...
    if (res < 0) {
      log_e("getsockopt on fd %d, errno: %d, \"%s\"", sockfd, errno,
            strerror(errno));
      close(sockfd);
      return 0;
    }
    ...
    if (sockerr != 0) {
      log_e("socket error on fd %d, errno: %d, \"%s\"", sockfd, sockerr,
            strerror(sockerr));
      close(sockfd);
      return 0;
    }
...
size_t PPPOSClient::write(uint8_t data) { return write(&data, 1); }
...
size_t PPPOSClient::write(const uint8_t *buf, size_t size) {
  ...
  while (retry) {
    ...
    if (FD_ISSET(socketFileDescriptor, &set)) {
      res = lwip_send(socketFileDescriptor, (void *)buf, bytesRemaining,
                      MSG_DONTWAIT);
      if (res > 0) {
        totalBytesSent += res;
